 ...
 */
namespace WPJoli\JoliClearLightBox\Controllers\Callbacks;
...
class SettingsCallbacks
{
    ...
    {
        return str_replace( array_map( 'jclb_mustache_key', array_keys( $data ) ), array_values( $data ), $template );
    }
    ...
    public function inputField( $args )
    {
        ...
        $data = [
            ...
            'option'      => $option,
            'value'       => jclb_get_option( $option[1], $option[0] ),
        ];
        // var_dump(jclb_get_option($option[1], $option[0]));
        ...
        if ( isset( $args['img'] ) && $args['img'] ) {
            $img_path = JCLB()->path( 'assets/admin/img/' . $args['img'] );
            $img_url = JCLB()->url( 'assets/admin/img/' . $args['img'] );
        ...
        if ( isset( $args['media'] ) ) {
            ...
            foreach ( $args['media'] as $media_id => $media_name ) {
                $output .= sprintf(
                    ...
                    JCLB()->url( 'assets/admin/img/' . $media_name )
                );
            }
            $output .= '</p>';
        }
